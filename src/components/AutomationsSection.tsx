  ...
  return <>
      ...
      <section id="automacoes" className="py-12 sm:py-16 lg:py-24 bg-gradient-to-br from-[#0C1F3F] to-[#1C1C1C] relative overflow-hidden">
        ...
        <div className="absolute inset-0 overflow-hidden">
          {Array.from({
          length: 50
        }).map((_, i) => <div key={i} className="absolute rounded-full bg-[#3A8DFF] opacity-20 animate-pulse" style={{
        ...
        <div className="absolute inset-0 opacity-10">
          <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
            ...
            {Array.from({
            length: 8
          }).map((_, i) => <line key={i} x1={`${Math.random() * 100}%`} y1={`${Math.random() * 100}%`} x2={`${Math.random() * 100}%`} y2={`${Math.random() * 100}%`} stroke="url(#bgLines)" strokeWidth="1" />)}
          </svg>
        </div>
